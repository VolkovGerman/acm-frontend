import React from 'react';

import Block from '../Layouts/BlockComponent/Block';
import WidgetTable from '../Widgets/WidgetTableComponent/WidgetTable';
import config from '../../../core/config/general.config';

const PUBLISH_STATUS = 1;

class ChampList extends React.Component {
    constructor(props) {
        super(props);

        this.state = {
            table: {},
            buttons: [
                {
                    action: '/champs/create',
                    name: 'Добавить',
                    type: 'link',
                    style: 'green'
                }
            ],
            actions: {
                delete: `${config.server}/champs/delete`
            }
        };
    }

    componentDidMount = () => {
        this.props.updateBlockTitle('Список чемпионатов');
        this.loadChamps();
    };

    componentWillUnmount = () => {
        this.props.setLoader();
    };

    loadChamps = () => {
        fetch(`${config.server}/champs`, {
            method: 'get',
        })
            .then(_ => _.json())
            .then(champs => {
                this.setState({
                    table: {
                        fields: [
                            'Название (рус.)',
                            'Название (анг.)',
                            'Год',
                            'Добавление'
                        ],
                        data: champs['_embedded']['champs'].map(_ => {
                                let createdAt = new Date(_.createdAt);
                                createdAt = `${createdAt.toLocaleDateString()}`;
                                return {
                                    id: _.id,
                                    actions: [
                                        {
                                            name: 'Изменить',
                                            link: `/champs/update?id=${_.id}`
                                        },
                                        {
                                            name: 'Просмотр секций',
                                            link: `/champs/${_.id}/sections`
                                        }
                                    ],
                                    cells: [
                                        _.titleRU,
                                        _.titleEN,
                                        _.year,
                                        createdAt
                                    ]
                                }

                            }
                        )
                    }
                });
                this.props.updateLoadedStatus(true, 1);
            });
    };

    render = () => {
        if (this.props.isLoader()) {
            return (
                <div className="Champs">
                    <Block title="Список чемпионатов" showButtons buttons={this.state.buttons}>
                        <WidgetTable table={this.state.table} actions={this.state.actions}/>
                    </Block>
                </div>
            )
        } else {
            return <div></div>
        }
    }

}

export default ChampList;
